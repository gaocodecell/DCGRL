...
def load_hete_data(datasets):
    ...
    percls_trn = int(round(0.1 * N / num_class))
    val_lb = int(round(0.1 * N))

    train_idx, val_idx, test_idx = permute_masks(N, labels, num_class, percls_trn, val_lb, 77)
...
def load_other_data(datasets):
    #g, labels, idx_train, idx_val, idx_test
    if datasets == 'photo':
        torch_dataset = AmazonCoBuyPhotoDataset()
    elif datasets == 'computer':
        torch_dataset = AmazonCoBuyComputerDataset()
    elif datasets == 'cs':
        torch_dataset = CoauthorCSDataset()
    elif datasets == 'physics':
        torch_dataset = CoauthorPhysicsDataset()

    graph = torch_dataset[0]
...
def get_train_val_test_split(
    ...
    if train_examples_per_class is not None:
        ...
        train_sum = train_labels.sum(0)
        # assert all classes have equal cardinality
        assert np.unique(train_sum.detach().cpu().numpy()).size == 1

    if val_examples_per_class is not None:
        val_labels = labels[val_indices, :]
        val_sum = val_labels.sum(0)
        # assert all classes have equal cardinality
        assert np.unique(val_sum.detach().cpu().numpy()).size == 1

    if test_examples_per_class is not None:
        test_labels = labels[test_indices, :]
        test_sum = test_labels.sum(0)
        # assert all classes have equal cardinality
        assert np.unique(test_sum.detach().cpu().numpy()).size == 1
