...
parser.add_argument('--dprate', type=float, default=0.3,  help='Dprate  (1 - keep probability).')
...
parser.add_argument('--eps', type=float, default=0.2,   help='the superparamter for FAGCN')
...
parser.add_argument('--net', type=str, default='GCN',  help='dataset to be used')
...
def test():
    ...
    return  acc_test, acc_val, acc_train, loss_train, loss_val,loss_test
...
for epoch in tqdm(range(args.epochs)):
    train()
    tmp_test_acc, val_acc, acc_train, loss_train, loss_val, loss_test = test()
    if epoch%2==0:
        filename = f'overfit/{args.dataset}_{args.net}_loss.csv'
        with open(f"{filename}", 'a+') as write_obj:
            write_obj.write("{},{},{}".format(loss_train, loss_val,loss_test))
            write_obj.write("\n")

        file= f'overfit/{args.dataset}_{args.net}_acc.csv'
        with open(f"{file}", 'a+') as write_obj:
            write_obj.write("{},{},{}".format(acc_train,val_acc,tmp_test_acc))
            write_obj.write("\n")
...
print(test_acc)
filename = f'overfit.csv'
print(f"Saving results to {filename}")
with open(f"{filename}", 'a+') as write_obj:
    write_obj.write( "dataset:{}, net:{},  trainrate:{} ,  testacc:{} ".format(args.dataset, args.net, args.train_rate, test_acc))
    write_obj.write("\n")
