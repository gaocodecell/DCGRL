...
parser.add_argument('--net', type=str, default='GraphMLP',
                    help='dataset to be used')
...
adj, features, labels, idx_train, idx_val, idx_test =get_data(args)
...
def Ncontrast(x_dis, adj_label, tau = 1):
    ...
    ####   losspos = -torch.log(x_dis_sum_pos).mean()  #only the postive samples
...
def train():
    ...
    model.train()
    optimizer.zero_grad()
    output, x_dis = model(features_batch,mask)
...
print(test_acc)
filename = f'GMLP/{args.net}_Negitem.csv'
print(f"Saving results to {filename}")
with open(f"{filename}", 'a+') as write_obj:
    write_obj.write(
            "dataset:{}, hidden:{}, lr:{}, hop:{}, belta:{}, tau:{}, meanmicro:{} ".format(args.dataset, args.hidden, args.lr,args.order, args.belta, args.tau, test_acc))
    write_obj.write("\n")
